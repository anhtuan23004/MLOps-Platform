...
This repository uses Harness v0. MLOps-Platform currently contains inherited
local model infrastructure plus a blank product harness.

The current job of agents is to preserve and grow the collaboration harness
while changing infrastructure only through selected stories. Do not scaffold
unrelated application source folders, platform shells, package scripts, CI, or
tests unless a story explicitly moves that area into implementation.
...
This harness does not ship with a project-specific `SPECS.md` contract. Product docs,
stories, tests, and decisions are the living contract that agents should update
